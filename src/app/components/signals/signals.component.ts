...
export class SignalsComponent {
  ...
  addToCart(product: Product, index: number): void {
    ...
    this.totalQuantity.update((qty: any) => qty + this.quantity());
    this.totalPrice.update((price: any) => price + this.quantity() * product.price);
    // this.products.update(
    //   (product: any) => (product.quantity = product[index].quantity - this.quantity())
    // );
    // this.cart.update((value: any) =>
    //   value.push({
    //     id: product.id,
    //     name: product.title,
    //     qty: this.quantity(),
  ...
  deleteCartItem(index: number) {
    // this.cart.mutate((v: any) => v.splice(index, 1));
  }
}
